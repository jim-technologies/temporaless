...
- Repository layout follows a language namespace:
  ...
  - `core/ts/`: TypeScript client SDK (generated protobuf, ConnectRPC store/query wrappers, visual-plan helpers, and the optional invariantprotocol projection); it is not a workflow runtime. Its npm entry is the root `package.json` so Git installs work without a registry.
  - `core/rs/`: experimental Rust SDK (storage and a minimal workflow runtime). The root `Cargo.toml` is a one-member workspace so Cargo can install the crate from Git.
...
- The ConnectRPC record-store boundary is deliberately asymmetric. Go ships it as the `adapters/go/connectstore` adapter, so the hand-written Go core packages never import connect-go. Python ships `ConnectStore`, `RecordStoreService`, and `asgi_application` in the core module `temporaless.connectstore`, because the ConnectRPC ASGI app is the Python core's serving surface and `connectrpc` is already a core dependency.
