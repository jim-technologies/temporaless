...
async def test_connect_store_deliver_event_propagates_typed_outcomes(tmp_path) -> None:
    store = ConnectStore.local(OpenDALStore(opendal.AsyncOperator("fs", root=str(tmp_path))))
    ...
    with pytest.raises(EventDeliveryConflictError) as captured:
        await deliver_event(store, key, StringValue(value="rejected"))
    assert captured.value.key == key


async def test_connect_store_treats_unspecified_event_delivery_as_unsupported(
    tmp_path,
) -> None:
    class UnspecifiedEventStore(OpenDALStore):
        delivery_calls = 0

        async def event_delivery_capability(
            self,
        ) -> temporaless_pb2.EventDeliveryCapability:
            return temporaless_pb2.EVENT_DELIVERY_CAPABILITY_UNSPECIFIED
        ...
        async def deliver_event(
            ...
            return await super().deliver_event(record)

    backend = UnspecifiedEventStore(opendal.AsyncOperator("fs", root=str(tmp_path)))
    ...
    assert backend.delivery_calls == 0


async def test_connect_store_ignores_unrelated_failed_precondition_detail() -> None:
    class UnrelatedDetailClient:
        async def deliver_event(
            self, _request: temporaless_pb2.DeliverEventRequest
            ...
            raise ConnectError(
                ...
                details=[
                    ConnectAny(
                        ...
                        value=StringValue(value="not an event detail").SerializeToString(
                            deterministic=True
                        ),
                    )
                ],
    ...
    store = ConnectStore(UnrelatedDetailClient())  # type: ignore[invalid-argument-type]
    with pytest.raises(ConnectError) as captured:
        await store.deliver_event(
...
@pytest.mark.parametrize(
    ...
    [
        ConnectAny(
            type_url="type.googleapis.com/temporaless.v1.EventDeliveryErrorDetail",
            value=temporaless_pb2.EventDeliveryErrorDetail(
                reason=temporaless_pb2.EVENT_DELIVERY_FAILURE_REASON_CONFLICT,
        ...
        ConnectAny(
            type_url="type.googleapis.com/temporaless.v1.EventDeliveryErrorDetail",
            value=temporaless_pb2.EventDeliveryErrorDetail(
                reason=temporaless_pb2.EVENT_DELIVERY_FAILURE_REASON_UNSPECIFIED,
                key=EventKey(
                    workflow_id="workflow",
                    run_id="run",
                    event_id="approval",
    ...
    ids=["invalid-key", "mismatched-key", "invalid-reason", "undecodable"],
)
async def test_connect_store_rejects_malformed_event_delivery_detail(
    detail: ConnectAny,
) -> None:
    class MalformedDetailClient:
        async def deliver_event(
            self, _request: temporaless_pb2.DeliverEventRequest
        ) -> temporaless_pb2.DeliverEventResponse:
            raise ConnectError(
    ...
    received_at = Timestamp()
    received_at.GetCurrentTime()
    store = ConnectStore(MalformedDetailClient())  # type: ignore[invalid-argument-type]
...
@pytest.mark.parametrize("missing", ["payload", "received_at"])
async def test_record_store_service_deliver_event_requires_complete_record(
    tmp_path,
    missing: str,
) -> None:
    ...
    record.ClearField(missing)

    with pytest.raises(ConnectError) as captured:
        await service.deliver_event(
            temporaless_pb2.DeliverEventRequest(record=record),
            None,
        )
    assert captured.value.code is Code.INVALID_ARGUMENT


async def test_connect_store_deliver_event_maps_corrupt_existing_record(
    tmp_path,
) -> None:
