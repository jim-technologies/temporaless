...
| [`adapters/go/dispatch`](adapters/go/dispatch) | Fire-and-forget dispatcher for gRPC-shaped handlers — `DoAsync(method, req)` + `Shutdown(ctx)` with a 15s default drain. The default queue is in-process and non-durable; a pluggable `Queue` can hand deterministic protobuf bytes to NATS JetStream/Kafka/RabbitMQ/SQS, whose native acknowledgement policy owns durability. |

Python equivalents for the operations adapters live in `core/py/src/temporaless/{timerscanner,cronscheduler,inspector,visualization,janitor,backfill,dependencies,dispatch}.py` (no separate uv project — they have no third-party deps). `visualization` validates/digests optional plans and overlays them on run records without pretending to infer code flow; `backfill` runs a workflow over many run_ids with bounded concurrency; `dependencies.wait_for_workflow` is the cross-pipeline durable wait primitive; `dispatch.Dispatcher` mirrors `adapters/go/dispatch` with a default asyncio task pool and a pluggable external `Queue` for deterministic protobuf handoff. Rust users get the same dispatch shape via `temporaless::dispatch`.
