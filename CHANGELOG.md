...
- Dependency currency: the TypeScript projection pins
  `@jim-technologies/invariant-protocol` 0.16.3 (commit `535d6ca5`) and
  moves to `@bufbuild/protobuf` 2.15 and `@connectrpc/connect` 2.2; Go moves
  to `buf.build/go/protovalidate` 1.4.0 (now on `cel.dev/cel-go`),
  `connectrpc.com/connect` 1.21.0, and `go.temporal.io/sdk` 1.49.0; the
  Python locks move to protobuf 7.36.2, `connectrpc` 0.12.1, and
  `protovalidate` 2.0.0, whose runtime is now `protobuf-py` instead of
  `cel-python` and `google-re2`; the strict adapters exact-pin `temporalio`
  1.33.0 and Prefect 3.8.6, and the Dagster process-boundary proof runs
  Dagster 1.13.24. Buf plugins move in lockstep (`connectrpc/go` 1.21.0,
  `protocolbuffers/python` and `pyi` 36.2, `bufbuild/es` 2.15.0) and every
  binding is regenerated.
- The Python core requires `connectrpc` 0.12.1 or newer: its Connect stubs
  are generated by `protoc-gen-connectrpc` 0.12.1 and import the 0.12 server
  API. Other declared floors are unchanged.
- The Flox manifest aligns with Invariant Protocol: schema 1.12.0, one lock
  for `aarch64-darwin`, `aarch64-linux`, and `x86_64-linux` (Intel macOS
  developers use native toolchains), Python 3.14.6, uv `>=0.11.26`, buf
  1.72.0, and Node 24.20.0, with the Go bootstrap and
  `GOTOOLCHAIN=go1.26.6+auto` unchanged. The production image moves to the
...
- `temporaless_prefectcompat.PrefectActivity` declares its type parameters
  inline so the request is contravariant and the response covariant, which
  `ty` 0.0.84 requires of a protocol; `PrefectActivity[Req, Resp]`
