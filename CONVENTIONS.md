...
to `scripts/validate` (Buf, TypeScript, Go, Python/uv, plus Rust when
installed). Go-only changes get fast feedback from individual Makefile
sub-targets (`fmt-check`, `tidy-check`, `vet`, `lint`, `test-go`). CI always
runs the separately pinned Rust 1.97 job. Both paths stay green.
...
| 7 | CI runs `flox activate -- make validate`. | **Conforms** | `.github/workflows/ci.yml` `full-gate` runs `flox activate -- make validate`; `go-check` runs the same Go sub-targets for fast branch feedback; `go-build` uses the same Flox Go/libffi environment; Rust has its own pinned job. |
...
- **Gate:** `flox activate -- make validate` is GREEN; the independent Rust
