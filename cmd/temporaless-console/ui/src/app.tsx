...
// shown in, the transport, and (in bearer mode) an in-memory token. main.tsx
// mounts it; the tests render it with a fake console behind `fetch`.
...
// BaseFetch is the platform fetch under that transport. The page leaves it
// unset (the transport then uses the global fetch); tests pass a fake console.
type BaseFetch = typeof globalThis.fetch

function consoleFetch(baseFetch: BaseFetch | undefined, onUnauthenticated?: (error: SourceError) => void): ProductFetch {
  return createProductFetch({ fetch: baseFetch, timeoutMs: REQUEST_TIMEOUT_MS, onUnauthenticated })
}
...
async function verifyToken(baseFetch: BaseFetch | undefined, token: string): Promise<void> {
  await consoleFetch(baseFetch)(TOKEN_CHECK_PATH, {
    method: 'POST',
...
function SignInGate({ refused, onSignedIn, fetch }: { refused?: SourceError, onSignedIn: (token: string) => void, fetch?: BaseFetch }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    setChecking(true)
    verifyToken(fetch, value).then(() => onSignedIn(value), (cause: unknown) => {
...
// refuseSession is the session after the console refused the token one
// request carried. A refusal answers that token only: once this tab holds
// another token, or none, a late 401 for the old one changes nothing.
export function refuseSession(current: Session, refusedToken: string | undefined, error: SourceError): Session {
  if (!refusedToken || current.token !== refusedToken) return current
  return { refused: error }
}

export function App({ config, fetch }: { config: UIConfig, fetch?: BaseFetch }) {
  ...
  // One transport per token, so each 401 is matched to the token its request
  // carried and a late refusal of a replaced token never signs the new one out.
  const transport = useMemo(() => consoleFetch(fetch, config.auth === 'bearer'
    ? error => setSession(current => refuseSession(current, token, error))
    : undefined), [fetch, config.auth, token])
...
// Bootstrap loads the console's UI configuration, then renders the app.
export function Bootstrap({ fetch }: { fetch?: BaseFetch }) {
  ...
  useEffect(() => {
    ...
    consoleFetch(fetch)('/ui/config', { cache: 'no-store', signal: controller.signal })
      .then(ensureOk)
  ...
  if (config) return <App config={config} fetch={fetch} />
  return (
