...
| Backfill across a date range | `await temporaless.backfill.backfill(invoke, run_ids, concurrency=N, halt_on_error=...)`. Bounded concurrency, aggregated `BackfillReport` with `succeeded()` / `failed()` / `pending()`, idempotent re-runs (COMPLETED replays from storage in microseconds). |
...
| Cross-DAG dependencies ("DAG B waits for DAG A's run") | `await temporaless.dependencies.wait_for_workflow(store, workflow_id="A", run_id=date, result_factory=...)`. Returns A's typed result on COMPLETED; raises `WorkflowDependencyPendingError` (workflow stays IN_PROGRESS) if A is unfinished; raises `WorkflowDependencyFailedError` if A failed terminally. |
| Per-run parameters (Airflow `dag_run.conf`) | The protobuf request message IS the parameter bag. Strongly typed, deterministic input digest. |
| Operator UI / re-run from UI | No UI. Use `inspector.list_workflows_by_status(FAILED)` + `inspector.reset_workflow(...)` from a script, notebook, or eventually invariantprotocol-generated CLI. |
